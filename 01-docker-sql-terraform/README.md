...
```sql
SELECT SUM(total_amount) AS sum_total_amount , z."Zone"
FROM public.green_taxi AS gt INNER JOIN public.zones AS z 
...
```sql
SELECT dz."Zone"
FROM public.zones AS pz INNER JOIN public.green_taxi AS gt
ON (pz."LocationID" = gt."PULocationID") INNER JOIN public.zones AS dz
ON (dz."LocationID" = gt."DOLocationID")
WHERE pz."Zone" = 'East Harlem North' AND gt."lpep_pickup_datetime" >= '2025-11-01' 
